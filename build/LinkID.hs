...
generateID md url author date
 ...
 where
  ...
  generateID' :: T.Text
  generateID'
    | null url' = url2ID (T.pack url) -- empty URL after stripping domain
...
authorsToCite url author date =
  let year = if date=="" then show CM.currentYear else take 4 date -- YYYY-MM-DD
      authors = filter (not . null) $ map (takeWhile (/= '#')) $ split ", " $ sedMany [(" \\([A-Za-z ]+\\)", ""), (" \\[[A-Za-z ]+\\]", "")] author -- affiliations like "Schizophrenia Working Group of the Psychiatric Genomics Consortium (PGC), Stephan Foo" or "Foo Bar (Atlas Obscura)" or /doc/math/humor/lion-hunting/1976-barrington.pdf's "John Barrington [Ian Stewart]" (the former is a pseudonym) would break the later string-munging & eventually the HTML
      authorCount = length authors
  in
       if authorCount == 0 || null authors then "" else
       let firstAuthorSurname = filter (\c -> isAlphaNum c || isPunctuation c) $ reverse $ takeWhile (/=' ') $ reverse $ deleteMany [" Senior", " Junior"] $ simplifiedHtmlToString $ head authors -- 'John Smith Junior 2020' is a weird cite if it turns into 'Junior 2020'! easiest fix is to just delete it, so as to get the expected 'Smith 2020'.
       in
           let
